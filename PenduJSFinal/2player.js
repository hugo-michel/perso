//le nombre de vie est codé "en dur" : let vie l82 (func ajoutMystery)
...
function ajoutMystery() {
    ...
    //input du mystery, stockage en LS, effacement du mystery
    let mystery = document.getElementById("mystery").value;
    mystery = mystery.toUpperCase();
    let regex = /^[a-zA-Z]+$/;
    let testRegex = regex.test(mystery);
    if (testRegex) {
        ...
        affichageGoal();
        affichageVie();
    } else {alert("le mot n'est pas valide, entrez un mot composée uniquement de lettres")}
